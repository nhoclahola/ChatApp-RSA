		...
		// Khi đã load hết tin nhắn thì mới chạy cái này
			...
			onValue(queryRef, async (snapshot) => {
				if (snapshot.exists()) {
					...
					setChatHistory(prevChatHistory => [newMessageObject, ...prevChatHistory])
				}
			})
		})
	}, [])
	...
	return (
		<TouchableWithoutFeedback onPress={Keyboard.dismiss}>
			<KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' || 'android' ? 'padding' : 'height'}>
				...
				></UIHeader>
				{!allLoaded ? <ActivityIndicator style={styles.loadingView} animating={!allLoaded}></ActivityIndicator> : null}
